...
mod tests {
    ...
    async fn test_empty_value() {
        ...
        let context = ResolvedContext::from_block(&var, None).await.unwrap();
        assert_eq!(context.variables.get("EMPTY_VAR"), Some(&"".to_string())); // I18N: no-translate - Rust assertion
    }
    ...
    async fn test_from_document_valid() {
        ...
        assert_eq!(var.id, id); // I18N: no-translate - Rust assertion
        assert_eq!(var.name, "TEST_VAR"); // I18N: no-translate - Rust assertion
        assert_eq!(var.value, "test_value"); // I18N: no-translate - Rust assertion
    }
    ...
    async fn test_from_document_missing_value_defaults_empty() {
        ...
        assert_eq!(var.id, id); // I18N: no-translate - Rust assertion
        assert_eq!(var.name, "TEST_VAR"); // I18N: no-translate - Rust assertion
        assert_eq!(var.value, ""); // I18N: no-translate - Rust assertion
    }
